 ...
 * @version 1.1
 * Sua nhung cho nao
 * De chan. em sua o dong 241 symtab.c*/
#include <stdio.h>
#include <stdlib.h>
...
void compileProgram(void) {
  ...
  eat(TK_IDENT);
    ...
    progObject = createProgramObject(currentToken->string);
  	//enter program block
...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      eat(TK_IDENT);
        Object* constObject = createConstantObject(currentToken->string);
      	//Create new constant object
      
      eat(SB_EQ);
      	//get constant value
      ConstantValue* value = compileConstant();  // MAYBE******************* XEM LAI
...
void compileBlock2(void) {                      // MAYBE******************* XEM LAI
  ...
  if (lookAhead->tokenType == KW_TYPE) {
    ...
    do {
      eat(TK_IDENT);
      Object* typeObject = createTypeObject(currentToken->string);
      ...
      typeObject->typeAttrs->actualType = type;
...
void compileBlock3(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      eat(TK_IDENT);
      Object* varObject = createVariableObject(currentToken->string);
      eat(SB_COLON);
      Type* type = compileType();
...
void compileFuncDecl(void) {                        // fuction vs proce co the co loi chua add paramlist
  ...
  funcObject->funcAttrs->returnType = type;
  eat(SB_SEMICOLON);
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    Object* tmp = lookupObject(currentToken->string);
    if(tmp == NULL)
    {
...
Type* compileType(void) {
  ...
  case TK_IDENT:
    ...
    else if(tmp->kind != OBJ_TYPE) 
    {
      error(ERR_INVALID_TYPE,lookAhead->lineNo, lookAhead->colNo);
    }
    else
    {
      type = duplicateType(tmp->typeAttrs->actualType);
    }
    break;
  default:
    error(ERR_INVALID_TYPE, lookAhead->lineNo, lookAhead->colNo);
...
void compileParam(void) {
  ...
  case TK_IDENT:
    ...
    eat(SB_COLON);
    paramObject->paramAttrs->type = duplicateType(compileBasicType());

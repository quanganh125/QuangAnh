...
void skipComment() {
  ...
  {
    readChar();
    if(charCodes[currentChar] == CHAR_RPAR) readChar(); 
    else skipComment();
  }
  else 
    {
...
Token* readIdentKeyword(void) {
  ...
  Token *token;
  char *string;
  string = (char*)malloc(sizeof(char)*MAX_IDENT_LEN);
  int i=0;

  while (charCodes[currentChar] != EOF && (charCodes[currentChar]== CHAR_LETTER || charCodes[currentChar]== CHAR_DIGIT|| currentChar=='_'))
  {
  ...
  if(checkKeyword(string) == TK_NONE) 
    ...
    token = makeToken(checkKeyword(string),lineNo,colNo);
...
Token* readNumber(void) {
  ...
  while (charCodes[currentChar] != EOF && charCodes[currentChar]== CHAR_DIGIT)
  {
...
Token* readConstChar(void) {
  ...
  readChar();
  if(charCodes[currentChar] != EOF && charCodes[currentChar]== CHAR_LETTER)
  {
    string[0]=currentChar;
    readChar();
    
    if(charCodes[currentChar] != EOF && charCodes[currentChar]== CHAR_SINGLEQUOTE)
      readChar();
    else error(ERR_INVALIDSYMBOL, lineNo, colNo);
  }
  else if(charCodes[currentChar] != EOF && currentChar== '\\')
  {
    string[0]=currentChar;
    readChar();
    if(charCodes[currentChar] != EOF && charCodes[currentChar]== CHAR_SINGLEQUOTE)
...
Token* getToken(void) {
  ...
  case CHAR_PERIOD:
    {   
      ...
      readChar(); 
      if(charCodes[currentChar] == CHAR_RPAR && charCodes[currentChar] != EOF) 
      {
  ...
  case CHAR_COLON:
    {
      readChar();
      if(charCodes[currentChar] == CHAR_EQ && charCodes[currentChar] != EOF)  
        {
  ...
  case CHAR_GT:
    ...
    case CHAR_LT:
    {
      ...
      else if(charCodes[currentChar] == CHAR_GT && charCodes[currentChar] != EOF)
      {
  ...
  case CHAR_LPAR:
     readChar(); 
     if(charCodes[currentChar] == CHAR_TIMES && charCodes[currentChar] != EOF)
      {
        readChar(); 
        skipComment();
        return getToken();
      }
      if(charCodes[currentChar] == CHAR_PERIOD && charCodes[currentChar] != EOF)
      {

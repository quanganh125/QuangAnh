...
public class DrawTower extends JPanel {
	...
	public void drawLayer(Tower tmp, int index) {
		...
		if (tmp == towerA)
			...
			this.xLocation = 505 - 15 * (8 - tmp.getLayerSize(index));
		else
			this.xLocation = 780 - 15 * (8 - tmp.getLayerSize(index));
	...
	public void reDraw(Tower from, Tower to) {
		towerWrapper.removeAll();
		drawFrame();
		System.out.println("test");
//		to.setLayerSize(to.getCurrentHeight(), from.getLayerSize(from.getCurrentHeight() - 1));
//		to.setCurrentHeight(to.getCurrentHeight() + 1);
//		if (from.getCurrentHeight() > 0)
//			from.setCurrentHeight(from.getCurrentHeight() - 1);
//		createTower(towerA);
//		createTower(towerB);
//		createTower(towerC);
		repaint();
